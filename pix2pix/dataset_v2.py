...
class Pix2PixDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            return {'A': augmentations["image"].transpose((2, 1, 0)).astype(np.float32),
                    'B': augmentations["image0"].transpose((2, 1, 0)).astype(np.float32), 'A_paths': str(self.before[index]), 'B_paths': str(self.after[index])}
        else:
            return {'A': before.transpose((2, 1, 0)).astype(np.float32),
                    'B': after.transpose((2, 1, 0)).astype(np.float32), 'A_paths':str(self.before[index]), 'B_paths': str(self.after[index])}
