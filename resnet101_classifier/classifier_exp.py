...
# ------------------ Helpers ------------------
def pad_to_multiple(x: torch.Tensor, multiple: int = 14) -> torch.Tensor:
...
def replace_first_conv(module: nn.Module, in_ch: int) -> bool:
    """
    ...
    for name, child in module.named_children():
        if isinstance(child, nn.Conv2d) and child.in_channels == 3:
            ...
            return True
        if replace_first_conv(child, in_ch):
            return True
    return False

def replace_classifier_head(model: nn.Module, num_classes: int, name: str):
    n = name.lower()
    if n.startswith("vit"):
...
def load_model_generic(name: str, in_ch: int, num_classes: int) -> nn.Module:
    ...
    if in_ch != 3:
        ok = replace_first_conv(model, in_ch)
        if not ok:
    ...
    replace_classifier_head(model, num_classes, name)
    return model
