...
class NumpyViewerApp(QWidget):
    ...
    def __init__(self):
        ...
        self.png_image = None  # Store tuple of (PNG image, RAW image)
        self.npy_image = None
            ...
            if self.images is not None:
                for file_name in os.listdir(folder_path):
                    if file_name.endswith(".npy"):
                        self.npy_image = np.load(os.path.join(folder_path, file_name))
                        self.png_image = plt.imread(str(self.images['image'][0]))

                self.bands = [float(band) for band in range(self.npy_image.shape[2])]
                self.shape_label1.setText(f"Loaded PNG and RAW images from Folder 1 with shape {self.png_image.shape}.")
                self.show_image()
            else:
    ...
    def show_image(self):
        ...
        if self.png_image is not None:
            self.ax1.imshow(self.png_image)
        ...
        if self.points and self.bands is not None:
            for i, (x, y) in enumerate(self.points):
                pixel_values = self.npy_image[int(y), int(x), :].flatten()
